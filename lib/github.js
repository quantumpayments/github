...
function insertResults(results) {
  ...
  config.database = 'gitpay';
  config.wallet = "https://localhost/wallet/gitpay/wallet#this";
  var credits = resultsToWebcredits(results);
  var sequelize = wc.setupDB(config);
  for (var i = 0; i < credits.length; i++) {
    console.log(credits[i]);
    wc.insert(credits[i], sequelize, config);
  }
}
...
function resultsToWebcredits(results) {
  ...
  for (i=0; i<results.length; i++) {
    var res = results[i];
    var cred = {};
    cred["https://w3id.org/cc#amount"] = 50;
    ...
    cred["https://w3id.org/cc#destination"] = user.login || user.email;
    ret.push(cred);
  }
